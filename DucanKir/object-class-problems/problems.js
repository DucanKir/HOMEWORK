...
make (string, eg: 'Apple')
model (string, eg: 'MacBook Pro')
ram (number, eg: 16)
storage (number, eg: 250)
*/
...
The product would be an instance of the Product class created above
*/
...
class Cart {
  ...
  removeItem (item) {
    this.contents = this.contents.filter((thing) => thing !== item)
  }
  empty () {
    this.contents = []
  }
...
class Triangle extends Shape {
  ...
  getPerimeter () {
    return this.height+this.width+Math.sqrt(this.height)+ Math.sqrt(this.width)
  }
}
